...
uint64_t interval_pcount;
uint64_t interval_icount;
uint64_t total_pcount;
uint64_t total_icount;

TaylorPoint* taylorpoint;

static bool emit_last_slice;
...
#ifdef VERBOSE
	if (__builtin_expect (total_pcount % PROGRESS_THRESHOLD == 0, false)) {
		ofstream progress_file;
		progress_file.open (mkfilename("mica_progress.txt"), ios::out | ios::trunc);
		progress_file << total_pcount << " instructions analyzed" << endl;
		progress_file.close ();
	}
#endif
}
...
inline VOID do_inst_count(INS ins){
    if(interval_size == -1)	{
...
VOID Trace(TRACE trace, VOID *v) {
    ...
    if (!RTN_Valid(r)){
        cerr << "Invalid RTN in Trace" << endl;
        return;
    }

    SEC s = RTN_Sec(r);
    if (!SEC_Valid(s)){
        cerr << "Invalid SEC in Trace" << endl;
        return;
    }

    IMG img = SEC_Img(s);
    if (!IMG_Valid(img)){
        cerr << "Invalid IMG in Trace" << endl;
        return;
    }

    if (!IMG_IsMainExecutable(img)){
        cerr << "Not main executable in Trace" << endl;
        return;
    }
...
VOID ThreadStart(THREADID tid, CONTEXT *ctxt, INT32 flags, VOID *v) {
    ...
    if (taylorpoint) {
        taylorpoint->profiles[tid]->OpenFile(tid, taylorpoint->OutputFile);
    }
}

VOID SimThreadFini(THREADID tid) {
    cerr << "Total icount: " << total_icount << endl;
    if (taylorpoint) {
        taylorpoint->Finish(tid);
    }
    if(emit_last_slice && interval_icount != (uint64_t)interval_size){
...
void printDebuggerCmd(int targetPid, LibInfo* libzsimAddrs) {
        /* return; */
        ...
        const char* const args[] = {"gdb", "-p", targetPidStr.c_str(),
            ...
            nullptr};
        FILE *f = fopen("/tmp/debug_zsim.sh", "w");
...
static EXCEPT_HANDLING_RESULT InternalExceptionHandler(THREADID tid, EXCEPTION_INFO *pExceptInfo, PHYSICAL_CONTEXT *pPhysCtxt, VOID *) {
    ...
    for (uint32_t i = 0; i < size; i++) {
        ...
        if (fname.find(QUOTED(LIBZSIM_NAME)) != std::string::npos) {
...
int main(int argc, char *argv[]) {
    ...
    Config config(KnobConfigFile.Value().c_str());

    procIdx = KnobProcIdx.Value();
    char header[64];
    snprintf(header, sizeof(header), "[S %d] ", procIdx);
    std::stringstream logfile_ss;
    const char* outputDir = config.get<const char*>("sim.outputDir", ".");
    logfile_ss << outputDir << "/zsim.log." << procIdx;
    InitLog(header, KnobLogToFile.Value()? logfile_ss.str().c_str() : nullptr);

    emit_last_slice  = config.get<bool>("sim.taylorsim.emit_last" , true);
    ...
    if (procIdx == 0 && !gm_isready()) {  // process 0 can exec() without fork()ing first, so we must check gm_isready() to ensure we don't initialize twice
        masterProcess = true;
        SimInit(config, outputDir, KnobShmid.Value(), KnobProcIdx.Value());
    } else {
...
#ifdef DEBUG_ZSIM
    printDebuggerCmd(syscall(SYS_getpid), &libzsimAddrs);
    sleep(5);
#endif
