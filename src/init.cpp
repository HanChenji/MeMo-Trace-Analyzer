...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    info("Built %s bank, %d bytes, %d lines, %d ways (%d candidates if array is Z), %s array, %s hash, %s replacement, accLat %d, invLat %d name %s",
            prefix.c_str(), bankSize, numLines, ways, candidates, arrayType.c_str(), hashType.c_str(), replType.c_str(), accLat, invLat, name.c_str());
...
DDRMemory* BuildDDRMemory(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string name, const string& prefix) {
    ...
    const char* tech = config.get<const char*>(prefix + "tech", "DDR4-2400-CL17");  // see cpp file for other techs
    ...
    uint32_t controllerLatency = config.get<uint32_t>(prefix + "controllerLatency", 40);  // in system cycles
...
static void InitSystem(Config& config) {
    ...
    for (auto& it : parentMap) {
        ...
        if (!found) panic("%s has invalid child %s\n", it.second.c_str(), it.first.c_str());
    }
    ...
    uint8_t llc_num = parentlessCacheGroups.size();
    if (llc_num > 1) panic("Only one last-level cache allowed, found: %s", Str(parentlessCacheGroups).c_str());
    string llc = llc_num ? parentlessCacheGroups[0] : string("");
    ...
    uint32_t memControllers = config.get<uint32_t>("sys.mem.controllers", 2);
    ...
    for (uint32_t i = 0; i < memControllers; i++) {
        ...
        mems[i] = BuildDDRMemory(config, zinfo->lineSize, zinfo->freqMHz, domain, name, "sys.mem.");
    }
    ...
    if (llc_num) {
        for (BaseCache* llcBank : (*cMap[llc])[0]) {
            llcBank->setParents(childId++, mems);
        }
    }
        ...
        for (uint32_t p = 0; p < parents; p++) {
            ...
            for (uint32_t c = p*childrenPerParent; c < (p+1)*childrenPerParent; c++) {
                for (BaseCache* bank : childCaches[c]) {
                    bank->setParents(childId++, parentsVec);
    ...
    OOOParams ooo_params;
    ooo_params.width                 = (uint32_t)config.get<int>("sys.cores.issue_width", 4);
    ooo_params.load_queue_cap        = (uint32_t)config.get<int>("sys.cores.load_queue_cap", 32);
    ooo_params.store_queue_cap       = (uint32_t)config.get<int>("sys.cores.store_queue_cap", 32);
    ooo_params.rob_cap               = (uint32_t)config.get<int>("sys.cores.rob_cap", 128);
    ooo_params.ins_win_cap           = (uint32_t)config.get<int>("sys.cores.ins_win_cap", 36);
    ooo_params.tage_num_tables       = (uint32_t)config.get<int>("sys.cores.tage_num_tables", 4);
    ooo_params.tage_index_size       = (uint32_t)config.get<int>("sys.cores.tage_index_size", 10);
    ooo_params.fetch_bytes_per_cycle = (uint32_t)config.get<int>("sys.cores.fetch_bytes_per_cycle", 4);

    ooo_params.prf_ports       = ooo_params.width * 2;
    ooo_params.issue_queue_cap = ooo_params.ins_win_cap; // set the same as the instruction window

    info("OOO width: %d", ooo_params.width);
    info("OOO prf port: %d", ooo_params.prf_ports)
    info("OOO load queue cap: %d", ooo_params.load_queue_cap);
    info("OOO store queue cap: %d", ooo_params.store_queue_cap);
    info("OOO rob cap: %d", ooo_params.rob_cap);
    info("OOO ins win cap: %d", ooo_params.ins_win_cap);
    info("OOO issue queue cap: %d", ooo_params.issue_queue_cap);
    info("OOO fetch bytes per cycle: %d", ooo_params.fetch_bytes_per_cycle);
    ...
    for (const char* group : coreGroupNames) {
        ...
        string type = config.get<const char*>(prefix + "type", "IssueModel");

        //Build the core group
        union {
            OOOCore*     oooCores;
            CacheModel*  cacheCores;
            FetchModel*  fetchCores;
            IssueModel*  issueCores;
            BbvProfiler* bbvProfilers;
        };

        if (type == "OOO") {
            oooCores = gm_memalign<OOOCore>(CACHE_LINE_BYTES, cores);
            zinfo->oooDecode = true;
        } else if (type == "CacheModel") {
            cacheCores = gm_memalign<CacheModel>(CACHE_LINE_BYTES, cores);
            zinfo->oooDecode = true; 
        } else if (type == "FetchModel") {
            fetchCores = gm_memalign<FetchModel>(CACHE_LINE_BYTES, cores);
            zinfo->oooDecode = true; 
        } else if (type == "IssueModel") {
            issueCores = gm_memalign<IssueModel>(CACHE_LINE_BYTES, cores);
            zinfo->oooDecode = true;
        }else if (type == "BbvProfiler") {
            bbvProfilers = gm_memalign<BbvProfiler>(CACHE_LINE_BYTES, cores);
            zinfo->oooDecode = false;
        } else {
        ...
        if (type == "OOO") {
            ...
            for (uint32_t j = 0; j < cores; j++) {
                ...
                //Build the core
                OOOCore* core = new (&oooCores[j]) OOOCore(ic, dc, ooo_params, name);
                zinfo->eventRecorders[coreIdx] = core->getEventRecorder();
            ...
            for (uint32_t j = 0; j < cores; j++) {
                ...
                //Build the core
                CacheModel* core = new (&cacheCores[j]) CacheModel(dc, ooo_params, name);
                zinfo->eventRecorders[coreIdx] = core->getEventRecorder();
                zinfo->eventRecorders[coreIdx]->setSourceId(coreIdx);
                coreMap[group].push_back(core);
                coreIdx++;
            ...
            for (uint32_t j = 0; j < cores; j++) {
                ...
                //Build the core
                FetchModel* core = new (&fetchCores[j]) FetchModel(ic, ooo_params, name);
                zinfo->eventRecorders[coreIdx] = core->getEventRecorder();
                zinfo->eventRecorders[coreIdx]->setSourceId(coreIdx);
                coreMap[group].push_back(core);
                coreIdx++;
            ...
            for (uint32_t j = 0; j < cores; j++) {
                ...
                IssueModel* core = new (&issueCores[j]) IssueModel(ooo_params, name);
                zinfo->eventRecorders[coreIdx] = core->getEventRecorder();
                zinfo->eventRecorders[coreIdx]->setSourceId(coreIdx);
                coreMap[group].push_back(core);
                coreIdx++;
            ...
            for (uint32_t j = 0; j < cores; j++) {
                ...
                BbvProfiler* core = new (&bbvProfilers[j]) BbvProfiler(name);
                coreMap[group].push_back(core);
                coreIdx++;
            }
...
void SimInit(Config config, const char* outputDir, uint32_t shmid, int32_t procId) {

    zinfo = gm_calloc<GlobSimInfo>();
    zinfo->outputDir = gm_strdup(outputDir);
    zinfo->statsBackends = new g_vector<StatsBackend*>();

    interval_size   = (int64_t)config.get<int>("sim.taylorsim.slice_size", 100000000);
    interval_icount = 0;
    interval_pcount = 0;
    total_icount    = 0;
    total_pcount    = 0;

    if (config.get<bool>("sys.taylorblock_profiling", false)) {
        taylorpoint = gm_memalign<TaylorPoint>(CACHE_LINE_BYTES);
        taylorpoint = new (taylorpoint) TaylorPoint(
            config.get<uint32_t>("sim.taylorsim.order", 0),
            config.get<bool>("sim.taylorsim.emit_first", true),
            config.get<bool>("sim.taylorsim.emit_last" , true),
            interval_size,
            std::string(outputDir) + "/taylorblock"
        );
    } else {
        taylorpoint = nullptr;
    }
    ...
    zinfo->freqMHz = config.get<uint32_t>("sys.frequency", 2500);
    ...
    zinfo->pinCmd = new PinCmd(&config, nullptr /*don't pass config file to children --- can go either way, it's optional*/, shmid);
    ...
    zinfo->profHeartbeats->init("heartbeats", "Per-process heartbeats", zinfo->lineSize /*max procs*/);
