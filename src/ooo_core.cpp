...
OOOCore::OOOCore(FilterCache* _l1i, FilterCache* _l1d, const OOOParams& ooo_params, g_string& _name) : Core(_name), l1i(_l1i), l1d(_l1d), ooo_width(ooo_params.width), ooo_prf_ports(ooo_params.prf_ports), fetch_bytes_per_cycle(ooo_params.fetch_bytes_per_cycle), cRec(0, _name) {
    ...
    // branchPred = new BranchPredictorPAg(ooo_params.PAg_bhr_log_cap, ooo_params.PAg_bhr_bits, ooo_params.PAg_pht_log_cap);
...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo, THREADID tid) {
    if (!prevBbl) {
